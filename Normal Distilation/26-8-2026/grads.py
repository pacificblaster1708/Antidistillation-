...
ADS and EMARC runs need this stage. Standalone ADS example:
...
def compute_grads(cfg: Config) -> dict:
    ...
    if not os.path.exists(cfg.holdout_traces):
        raise SystemExit(
            f"holdout traces not found at {cfg.holdout_traces}; run the holdout "
    ...
    model = load_causal_lm(
        cfg.proxy_student, torch.float32, resolve_attn(cfg.attn_impl), use_cache=False
    )
    vocab = align_vocab(tokenizer, model)
    if main:
        n_params = sum(p.numel() for p in model.parameters())
        print(
            f"[grads] {cfg.proxy_student}: {n_params / 1e6:.1f}M params, vocab {vocab}"
        )
    ...
    def build():
        ...
        out = traces.map(
            ...
            desc="Tokenizing holdout traces",
        )
        return out.filter(
    ...
    tokenized = build_on_main(
        accelerator,
        os.path.join(cfg.exp_dir, ".cache"),
        f"grads|{cfg.holdout_traces}|{cfg.teacher}|{column}|{cfg.train_max_length}",
        build,
    )
    if len(tokenized) == 0:
        raise SystemExit(
            "every holdout trace has an empty completion; nothing to differentiate"
        )
