...
Used four times by run.py, with different flags each time:
  ...
  train traces     teacher, plain/ADS/EMARC -> what the student is distilled from
...
from emarc import EMARCLogitsProcessor, merge_emarc_stats
...
def _resolve_stage(cfg: Config) -> dict:
    ...
    split = cfg.gen_split
    if cfg.gen_method != "auto":
        method = cfg.gen_method
    elif cfg.gen_use_ads == "auto":
        method = (
            cfg.mode if cfg.mode in {"ads", "emarc"} and split == "train" else "plain"
        )
    else:
        enabled = to_bool(cfg.gen_use_ads)
        method = (
            cfg.mode
            if enabled and cfg.mode in {"ads", "emarc"}
            else ("ads" if enabled else "plain")
        )
    use_ads = method in {"ads", "emarc"}
    ...
    out = (
        cfg.gen_out
        or {
            "train": cfg.train_traces,
            "holdout": cfg.holdout_traces,
            "test": os.path.join(cfg.traces_dir, "test"),
        }[split]
    )
    return {
        "model": model,
        ...
        "out": out,
        "label": cfg.gen_label or f"{split}/{method}",
    }
...
def generate_traces(cfg: Config) -> dict:
            ...
            json.dumps(
                {
                    **{k: v for k, v in stage.items()},
                    ...
                    "lam": cfg.lam,
                    "eps": cfg.eps,
                    "emarc_alpha_init": cfg.emarc_alpha_init,
                },
                indent=2,
            ),
        )
    ...
    if stage["use_ads"]:
        direction_path = (
            cfg.direction_path if stage["method"] == "emarc" else cfg.grad_path
        )
        if not os.path.exists(direction_path):
            raise SystemExit(
                f"{stage['method'].upper()} needs a proxy-student direction; "
                f"{direction_path} does not exist. Run the gradient/direction "
                f"stages first."
            )
        grads, meta = load_grads(direction_path)
        plus_model = load_causal_lm(
            cfg.proxy_student, dtype, attn, device=accelerator.device
        )
        minus_model = load_causal_lm(
            cfg.proxy_student, dtype, attn, device=accelerator.device
        )
        vocab = align_vocab(tokenizer, model, plus_model, minus_model)
        if meta.get("vocab_size") not in (None, vocab):
            raise SystemExit(
                f"gradients were computed with vocab_size={meta['vocab_size']} but "
                f"this tokenizer has {vocab} tokens"
            )
        # Resizing can append randomly initialized vocabulary rows. Make the
        # central-difference endpoints identical before applying +/- eps*v.
        minus_model.load_state_dict(plus_model.state_dict(), strict=True)
        ...
        if main:
            ...
            print(
                f"[{stage['method']}] param RMS {stats_p['param_rms']:.3e} "
                f"| direction RMS {stats_p['grad_rms']:.3e} "
            ...
            if dtype in (torch.float16, torch.bfloat16):
                ...
                if step < stats_p["param_rms"] * resolution:
                    print(
                        f"[{stage['method']}] WARNING: eps*direction is small next to "
                        f"{dtype} resolution; the two "
    ...
    def build():
        ...
        workers = cfg.map_workers if len(raw) >= 1024 and cfg.map_workers > 1 else None
        out = raw.map(
            tokenize,
            batched=True,
            batch_size=512,
        ...
        before = len(out)
        out = out.filter(
            lambda x: x["prompt_len"] <= cfg.max_prompt_length,
            desc="Dropping over-long prompts",
        )
        if len(out) < before:
            print(
                f"[gen] dropped {before - len(out)} prompt(s) longer than "
                f"{cfg.max_prompt_length} tokens"
            )
        return out

    key = (
        f"gen|{cfg.dataset}|{stage['split']}|{stage['tokenizer']}|"
    ...
    gen_kwargs = dict(
        ...
        # EMARC implements the nominal top-p support internally so its KL and
        # typicality diagnostics describe the distribution actually sampled.
        top_p=(1.0 if stage["method"] == "emarc" else cfg.top_p)
        if cfg.tau > 0
        else None,
        top_k=None,
        use_cache=True,
    ...
    local_emarc_stats: List[dict] = []
    for batch in tqdm(
        loader, total=len(loader), desc=f"generate[{stage['label']}]", disable=not main
    ):
        ...
        if stage["method"] == "ads":
            ...
            processors = LogitsProcessorList(
                [
                    ADSLogitsProcessor(
                        plus, minus, cfg.lam, cfg.eps, batch["attention_mask"]
                    )
                ]
            )
        elif stage["method"] == "emarc":
            plus.reset()
            minus.reset()
            emarc_processor = EMARCLogitsProcessor(
                plus,
                minus,
                cfg.eps,
                ...
                temperature=cfg.tau,
                top_p=cfg.top_p,
                alpha_init=cfg.emarc_alpha_init,
                alpha_min=cfg.emarc_alpha_min,
                alpha_max=cfg.emarc_alpha_max,
                alpha_lr=cfg.emarc_alpha_lr,
                target_flow=cfg.emarc_target_flow,
                antiwindup=cfg.emarc_antiwindup,
                sigma=cfg.emarc_sigma,
                attacker_eta=cfg.emarc_attacker_eta,
                attacker_iters=cfg.emarc_attacker_iters,
                mass_min=cfg.emarc_mass_min,
                mass_max=cfg.emarc_mass_max,
                value_source=cfg.emarc_value_source,
                value_floor=cfg.emarc_value_floor,
                beta_max=cfg.emarc_beta_max,
                kl_cap=cfg.emarc_kl_cap,
                typicality_kappa=cfg.emarc_typicality_kappa,
                max_backtracks=cfg.emarc_max_backtracks,
                margin=cfg.emarc_margin,
                entropy_threshold=cfg.emarc_entropy_threshold,
                margin_gamma=cfg.emarc_margin_gamma,
                margin_mass_min=cfg.emarc_margin_mass_min,
                safe_top_k=cfg.emarc_safe_top_k,
                safe_logit_gap=cfg.emarc_safe_logit_gap,
                protected_z=cfg.emarc_protected_z,
                min_score_gap=cfg.emarc_min_score_gap,
                margin_allow_kl_override=cfg.emarc_margin_allow_kl_override,
                eos_token_id=tokenizer.eos_token_id,
                protected_exclude_ids=tokenizer.all_special_ids,
            )
            processors = LogitsProcessorList([emarc_processor])
        ...
        for row in out[:, prompt_width:]:
            completions.append(
                strip_terminals(
                    tokenizer.decode(row, skip_special_tokens=False), tokenizer
                ).rstrip()
            )
        if stage["method"] == "emarc":
            local_emarc_stats.append(emarc_processor.totals())

    shard = shard.add_column("completion", completions)
    all_emarc_stats = (
        gather_object(local_emarc_stats) if stage["method"] == "emarc" else []
    )
    ...
    def score(example):
        ...
        if cfg.answer_force:
            out["is_correct_af"] = is_correct(
                example["completion_af"], example["solution"]
            )
        return out
    ...
    summary: dict = {}
    if main:
        merged = concatenate_datasets([load_from_disk(p) for p in all_paths]).sort(
            "idx"
        )
        ...
        if stage["method"] == "emarc":
            summary["controller"] = merge_emarc_stats(all_emarc_stats)
...
def _answer_force(cfg: Config, accelerator, tokenizer, model, shard, main) -> List[str]:
    ...
    results: List[str] = []
    for start in tqdm(
        range(0, len(items), batch_size), desc="answer-force", disable=not main
    ):
        ...
        with torch.inference_mode():
            out = model.generate(
                **batch,
                ...
                top_p=None,
                top_k=None,
                max_new_tokens=cfg.answer_force_tokens,
                use_cache=True,
                pad_token_id=tokenizer.pad_token_id,
                eos_token_id=tokenizer.eos_token_id,
            )
        for item, row in zip(chunk, out[:, width:]):
            finish = strip_terminals(
                tokenizer.decode(row, skip_special_tokens=False), tokenizer
            )
