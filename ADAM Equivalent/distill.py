...
from ads import (CompletionOnlyCollator, align_vocab, banner, build_on_main, describe,
                 init_runtime, load_causal_lm, load_tokenizer, resolve_attn, resolve_dtype)
...
def _build_dataset(cfg: Config, state, tokenizer, path: str, column: str, desc: str):
    ...
    def build():
        out = traces.map(tokenize, batched=True, batch_size=512,
                         ...
                         remove_columns=traces.column_names, desc=desc)
        return out.filter(lambda x: len(x["input_ids"]) > x["prompt_len"],
                          desc="Dropping empty completions")

    return build_on_main(state, os.path.join(cfg.exp_dir, ".cache"),
                         f"distill|{path}|{cfg.student_tokenizer}|{column}|"
...
def distill(cfg: Config) -> dict:
    ...
    if main:
        banner(f"DISTILLATION [{cfg.mode}]",
               f"student : {cfg.student}\n"
               f"traces  : {cfg.train_traces}\n"
               f"output  : {cfg.student_final}")

    tokenizer = load_tokenizer(cfg.student_tokenizer or cfg.student, padding_side="right")
    dtype = resolve_dtype(cfg.dtype)

    model = load_causal_lm(cfg.student, dtype, resolve_attn(cfg.attn_impl), use_cache=False)
    ...
    if cfg.lora:
        ...
        model = get_peft_model(model, LoraConfig(
            ...
            target_modules=_lora_targets(model), bias="none", task_type="CAUSAL_LM",
        ))
        if main:
            model.print_trainable_parameters()

    column = "completion_af" if cfg.train_on_answer_forced else "completion"
    train_ds = _build_dataset(cfg, state, tokenizer, cfg.train_traces, column,
                              "Tokenizing train traces")
    eval_ds = None
    if cfg.do_eval and os.path.exists(cfg.holdout_traces):
        eval_ds = _build_dataset(cfg, state, tokenizer, cfg.holdout_traces, "completion",
    ...
    if main and effective != cfg.train_batch_size:
        print(f"[distill] WARNING: train_batch_size={cfg.train_batch_size} is not reachable with "
